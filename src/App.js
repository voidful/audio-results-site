...
export default function ResultsSite() {
  ...
  const [samples, setSamples] = useState([]); // 正規化後的 {audioPaths, prompt, prediction, label, correct, index}
  ...
  // 依據 Vite 的 BASE_URL 推導在 GitHub Pages 子路徑下的預設 Base URL
        ...
        <section className="bg-white rounded-2xl shadow p-4 md:p-6 border border-neutral-200">
          <details>
            ...
            <ol className="mt-3 list-decimal pl-5 space-y-2 text-sm text-neutral-700">
              <li>把本檔案丟進任何 React/Vite 專案（src/App.jsx 或任一頁面），或用 Codesandbox 直接跑。</li>
              <li>把所有 .wav 放到網站可公開讀取的位置（例如 public/audio/），再用上方「URL 生成模式」把檔名轉為對應 URL。</li>
              <li>若你的評測輸出很大，建議預先在伺服器做分頁；此頁面為前端離線運算，10k 筆級別仍可使用。</li>
            </ol>
          </details>
        </section>
      </main>
